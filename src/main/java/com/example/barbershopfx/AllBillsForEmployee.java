...
public class AllBillsForEmployee implements Initializable {
    ...
    public static class Item{
        ...
        public Item(String billNumber, String employeeName, String cashierName, String cash, String time) {
    ...
    public void LoadTheItemsTable(){
        try {
            ...
            if(mode==0){
                billStmt =
                        connection.prepareStatement("SELECT * FROM bills WHERE worker_user_name = ? AND last_time_updated >= ? AND last_time_updated < ?");
            }else if(mode==1){
                billStmt =
                        connection.prepareStatement("SELECT * FROM bills WHERE customer_barcode = ? AND last_time_updated >= ? AND last_time_updated < ?");
            }else{
                billStmt =
                        connection.prepareStatement("SELECT * FROM bills WHERE cashier_user_name = ? AND last_time_updated >= ? AND last_time_updated < ?");
            }
            ...
            billStmt.setString(1, myText);
            billStmt.setTimestamp(2, timestampStart);
            billStmt.setTimestamp(3, timestampEnd);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if(mode==0){
            nameText.setText(" الموظف : "+myText);
        }else if(mode==1){
            nameText.setText(" العميل رقم : "+myText);
        }else{
            nameText.setText(" الكاشير : "+myText);
        }
